...
#excel中提前数据
cases = get_test_data_from_excel(settings.TEST_DATA_FILE,'login')
@ddt
class TestLogin(unittest.TestCase):
    logger = logger
    @classmethod
    def setUpClass(cls) -> None:
       cls.logger.info("====登录接口开始测试>>>>>>>>>>")
    @classmethod
    def tearDown(cls) -> None:
        cls.logger.info("====登录接口测试结束<<<<<<<<<<")
    ...
    def test_login(self,case):
        ...
        try:
            ...
            self.logger.exception('请求结果断言失败!',e)
